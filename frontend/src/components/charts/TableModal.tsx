...
interface TableModalProps {
  ...
  title: string;
  children: ReactNode;
  dataColumns?: number; // Number of data columns to calculate width
}

export default function TableModal({ isOpen, onClose, title, children, dataColumns = 12 }: TableModalProps) {
  ...
  // Calculate minimum width based on data columns (min 100px per column + 200px for labels)
  const minTableWidth = Math.max(dataColumns * 100 + 200, 800);
  ...
  return createPortal(
    ...
    >
      ...
      {/* Modal content */}
      <div 
        className="relative w-full max-w-[95vw] h-[90vh] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-scale-in"
