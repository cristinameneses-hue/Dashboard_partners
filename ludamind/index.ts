...
import { generateDomainPrompt, QUERY_PATTERNS } from "./src/domain-context.js";
import {
  UNIFIED_TOOLS,
  updateToolSchemas,
  getUnifiedManager
} from "./src/mcp/unified-tools.js";
...
export default function createMcpServer({
  ...
  server.setRequestHandler(ListResourcesRequestSchema, async () => {
    ...
    `;
      ...
      // Add domain context resource
      resources.push({
        uri: "mysql://domain-context",
        name: "Domain Context",
        title: "Pharmacy Analytics Domain Context",
        description: "Comprehensive guide for translating natural language queries to SQL. Includes table descriptions, common query patterns, business rules, and examples.",
        mimeType: "text/markdown",
      });

      // Add query patterns resource
      resources.push({
        uri: "mysql://query-patterns",
        name: "Query Patterns",
        title: "Common Query Patterns",
        description: "Library of SQL query templates for common questions (sales, risk, orders, availability)",
        mimeType: "application/json",
      });
  ...
  server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    try {
      ...
      // Handle domain context resource
      if (request.params.uri === "mysql://domain-context") {
        const domainPrompt = generateDomainPrompt();
        return {
          contents: [
            {
      ...
      // Handle query patterns resource
      if (request.params.uri === "mysql://query-patterns") {
        return {
          contents: [
            {
              uri: request.params.uri,
              mimeType: "application/json",
              text: JSON.stringify(QUERY_PATTERNS, null, 2),
            },
          ],
        };
      }
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
      ...
      // Handle MySQL query tool (legacy)
      ...
      const unifiedTool = UNIFIED_TOOLS.find(tool => tool.name === request.params.name);
      if (unifiedTool) {
        return await unifiedTool.handler(request.params.arguments || {});
      }
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    // Update unified tool schemas with available databases
    updateToolSchemas();
  ...
  (async () => {
    try {
      ...
      log("info", "Initializing Unified Database Manager...");
      const unifiedMgr = getUnifiedManager();
      const databases = unifiedMgr.listDatabases();
      log("info", `Unified Manager ready with ${databases.length} databases`);
    } catch (error) {
